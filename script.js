...
.then(data => {
    ...
    if (!data || !data.players) {
        document.getElementById("server-status").innerText = "Status unbekannt";
        return;
    }
...
teamMembers.forEach(member => {
    ...
    card.innerHTML = `
        <img src="https://mc-heads.net/avatar/${member.name}" width="80" style="border-radius:10px;margin-bottom:10px;">
        <h3>${member.name}</h3>
